    ...
    protected String doInBackground(String... params) {
        try {
            ...
            if(verificarE){
                ...
                while (rs.next()) {
                    productos.add(new VentasProductoD(rs.getInt(1),rs.getInt(2),rs.getString(3),rs.getString(4),rs.getDouble(5),rs.getDouble(6),rs.getInt(7),rs.getInt(8),rs.getInt(9),rs.getString(10),rs.getString(11)));
                }
                rs = st.executeQuery("SELECT * FROM VentasProductos WHERE codigoV="+Codigo+" AND codigoP IS NULL");
                while (rs.next()) {
                    productos.add(new VentasProductoD(rs.getInt(1),rs.getInt(2),"null","Producto Desconocido",rs.getDouble(4),rs.getDouble(5),rs.getInt(6),rs.getInt(7),rs.getInt(8),rs.getString(9),rs.getString(10)));
                }
                return "";
            }
        }catch (Exception e){
