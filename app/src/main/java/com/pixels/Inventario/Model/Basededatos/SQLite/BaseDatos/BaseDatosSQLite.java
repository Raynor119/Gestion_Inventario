...
public class BaseDatosSQLite extends SQLiteOpenHelper {
    ...
    private static final String TABLAVenta="CREATE TABLE Venta(codigo INT NOT NULL,Fecha TEXT NOT NULL,Efectivo BIGINT NOT NULL,TotalV BIGINT NOT NULL,PRIMARY KEY (codigo))";
    private static final String TABLAVentasProductos="CREATE TABLE VentasProductos(Id INT AUTO_INCREMENT,codigoV INT NOT NULL,codigoP VARCHAR(255) NOT NULL,CantidadV INT NOT NULL,PRIMARY KEY (Id),FOREIGN KEY (codigoV) REFERENCES Venta(codigo),FOREIGN KEY (codigoP) REFERENCES Producto(codigo))";
