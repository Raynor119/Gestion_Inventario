...
public class BaseDatosSQLite extends SQLiteOpenHelper {
    ...
    private static final String TABLAVenta="CREATE TABLE Venta(codigo INTEGER PRIMARY KEY AUTOINCREMENT,Fecha TEXT NOT NULL,Efectivo BIGINT NOT NULL,TotalV BIGINT NOT NULL)";
    private static final String TABLAVentasProductos="CREATE TABLE VentasProductos(Id INTEGER PRIMARY KEY AUTOINCREMENT,codigoV INT NOT NULL,codigoP VARCHAR(255) NOT NULL,CantidadV DOUBLE NOT NULL,FOREIGN KEY (codigoV) REFERENCES Venta(codigo),FOREIGN KEY (codigoP) REFERENCES Producto(codigo))";
