...
public class VerificarCodigoVentaSQLite extends BaseDatosSQLite implements MediadorBaseDatos {
    ...
    public List<VentasProductoD> DatosProductos(){
        ...
        Cursor cursor=bd.rawQuery("SELECT VentasProductos.Id,VentasProductos.codigoV,VentasProductos.codigoP,Producto.nombre,VentasProductos.CantidadV,VentasProductos.CantidadD,Producto.TipoC,VentasProductos.CostePV,VentasProductos.PrecioPV,VentasProductos.IvaPV,VentasProductos.EstadoDevolucion,VentasProductos.ObservacionD FROM VentasProductos INNER JOIN Producto ON VentasProductos.codigoP=Producto.codigo WHERE VentasProductos.codigoV="+Codigo+"",null);
        if(cursor.moveToFirst()){
            do{
                datos.add(new VentasProductoD(cursor.getInt(0),cursor.getInt(1),cursor.getString(2),cursor.getString(3),cursor.getDouble(4),cursor.getDouble(5),cursor.getString(6),cursor.getInt(7),cursor.getInt(8),cursor.getInt(9),cursor.getString(10),cursor.getString(11)));
            }while(cursor.moveToNext());
        }
        cursor=bd.rawQuery("SELECT * FROM VentasProductos WHERE codigoV="+Codigo+" AND codigoP IS NULL",null);
        if(cursor.moveToFirst()){
            do{
                datos.add(new VentasProductoD(cursor.getInt(0),cursor.getInt(1),"null","Producto Desconocido",cursor.getDouble(3),cursor.getDouble(4),"null",cursor.getInt(5),cursor.getInt(6),cursor.getInt(7),cursor.getString(8),cursor.getString(9)));
            }while(cursor.moveToNext());
        }
        return datos;
    }
