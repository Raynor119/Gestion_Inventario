...
public class VerificarCreacionBaseDatos extends AsyncTask<String, Void , Boolean> {
    ...
    protected Boolean doInBackground(String... params) {
        try {
            ...
            st.execute("CREATE TABLE Venta(codigo INT AUTO_INCREMENT,Fecha DATETIME NOT NULL DEFAULT NOW(),Efectivo BIGINT NOT NULL,TotalV BIGINT NOT NULL,PRIMARY KEY (codigo))");
            st.execute("CREATE TABLE VentasProductos(Id INT AUTO_INCREMENT,codigoV INT NOT NULL,codigoP VARCHAR(255) NOT NULL,CantidadV DOUBLE NOT NULL,PRIMARY KEY (Id),FOREIGN KEY (codigoV) REFERENCES Venta(codigo),FOREIGN KEY (codigoP) REFERENCES Producto(codigo))");
            connection.close();
            return true;
        }catch (Exception e){
            return false;
