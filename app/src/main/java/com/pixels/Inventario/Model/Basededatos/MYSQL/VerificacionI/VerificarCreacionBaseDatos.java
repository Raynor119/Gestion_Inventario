...
public class VerificarCreacionBaseDatos extends AsyncTask<String, Void , Boolean> {
    ...
    protected Boolean doInBackground(String... params) {
        try {
            ...
            st.execute("CREATE TABLE VentasProductos(Id INT AUTO_INCREMENT,codigoV INT NOT NULL,codigoP VARCHAR(255),CantidadV DOUBLE NOT NULL,CantidadD DOUBLE NOT NULL,CostePV BIGINT NOT NULL,PrecioPV BIGINT NOT NULL,IvaPV INT NOT NULL,ObservacionD VARCHAR(255),PRIMARY KEY (Id),FOREIGN KEY (codigoV) REFERENCES Venta(codigo),FOREIGN KEY (codigoP) REFERENCES Producto(codigo) ON UPDATE CASCADE)");
            connection.close();
            return true;
        }catch (Exception e){
            return false;
